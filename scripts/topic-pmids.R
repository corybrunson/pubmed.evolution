...
# Save topic PMID vectors
save(topic.pmids, file = 'proj/pubmed.evolution/data/topic-pmids.RData')

# INTERSECT

# Load the intersected data
load('proj/pubmed.evolution/data/pmid-intersect.RData')

# Intersect the topic PMID vectors with the intersected PMID vectors
topic.pmids <- lapply(topic.pmids, function(vec) {
    intersect(vec, pmid.use$pmid)
})
